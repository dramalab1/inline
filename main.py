...
from telethon.errors.rpcerrorlist import PeerIdInvalidError, UserAlreadyParticipantError
...
async def helpp(e):
    if e.fwd_from:
        return
    button = Button.switch_inline("Search inline", query="keywords", same_peer=True)
...
async def _(e):
    ...
    async for x in bot.iter_messages(in_ch, search=search, wait_time=10):
        ...
        if not x.media:
            ...
            # link = (await bot(ExportMessageLinkRequest(ok.chat_id,
            # ok.id))).link
